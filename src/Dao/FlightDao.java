...
//            System.out.println(flight);
    ...
    @Test
    public List <FlightInformaiton> findOnewayFightNoStopFlex(Flight flight) {
        List <FlightInformaiton> existFlightList = new ArrayList<FlightInformaiton>();
        Connection conn = null;
        java.sql.PreparedStatement stmt = null;               
        ResultSet rs = null;
        try {
            conn = JdbcUtil.getConnection();
            String sql = "SELECT *,al.name AS airlinename FROM leg L, airport A, airport B,airline al WHERE L.DepAirportID = A.Id AND L.ArrAirportID = B.Id  AND A.City =? AND B.City =? AND al.id=L.AirlineID  AND TO_DAYS(DATE(L.DepTime))%7 <= TO_DAYS(DATE(?))%7 + 1 AND TO_DAYS(DATE(L.DepTime))%7 > TO_DAYS(DATE(?))%7 - 1";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, flight.getDepartCity());
            stmt.setString(2, flight.getArriveCity());
            stmt.setString(3,flight.getDepartDate());
            stmt.setString(4,flight.getDepartDate());
            System.out.println(flight);
            System.out.println(sql);
            rs = stmt.executeQuery();
    ...
    public List <FlightInformaiton> findOnewayFightOneStopFlex(Flight flight) {
        List <FlightInformaiton> existFlightList = new ArrayList<FlightInformaiton>();
        Connection conn = null;
        java.sql.PreparedStatement stmt = null;               
        ResultSet rs = null;
        try {
            conn = JdbcUtil.getConnection();
            String sql = "SELECT L.DepTime AS DepartTime1, L.ArrTime AS ArriveTime1, L2.DepTime AS DepartTime2, L.ArrTime AS ArriveTime2, A.Name AS Name1, B.Name AS Name2, C.Name AS Name3,al1.name AS airlinename1, al2.name AS airlinename2 FROM leg L, Leg L2, airport A, airport B, airport C, airline al1, airline al2 WHERE L.DepAirportID = A.id AND L2.ArrAirportID = B.id AND  A.city = ? AND  B.city =? AND al1.id=L.AirlineID AND al2.id=L2.AirlineID AND L.ArrAirportID = L2.DepAirportID AND C.id = L.ArrAirportID AND L.ArrTime < L2.DepTime AND TO_DAYS(DATE(L.DepTime))%7 <= TO_DAYS(DATE(?))%7 + 1 AND TO_DAYS(DATE(L.DepTime))%7 > TO_DAYS(DATE(?))%7 - 1";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, flight.getDepartCity());
            stmt.setString(2, flight.getArriveCity());
            stmt.setString(3, flight.getDepartDate());
